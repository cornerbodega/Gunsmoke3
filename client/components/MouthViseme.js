...
export default function MouthViseme({
  ...
  const texturePaths = {
    ...
    TH: "/mouth_visemes/v3/S.png",
    // TH: "/mouth_visemes/v3/TH.png",
    S: "/mouth_visemes/v3/L.png",
  ...
  const delta = 0.19; // Adjust this value to control the time window for viseme detection
  const getVisemeForTime = (time) => {
    ...
    for (let i = 0; i < visemeData.frames.length; i++) {
      ...
      if (time + delta >= frame.time && (!next || time + delta < next.time)) {
